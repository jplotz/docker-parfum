...
describe("Testing rule matcher", () => {
  ...
  test("curlUseFlagF valid", () => {
    const root = parseShell("curl -f https://");
    expect(new Matcher(root).match(curlUseFlagF)).toHaveLength(0);
  });
  ...
  test("npmCacheCleanAfterInstall inside binary", async () => {
    const root = parseDocker("RUN npm update && npm i");
    const matcher = new Matcher(root);
  ...
  test("rmRecursiveAfterMktempD valid", () => {
    const root = parseShell("RUN mktemp -d fold && rm -rf fold\n");
  ...
  test("curlUseHttpsUrl invalid3", async () => {
    const root = parseShell('curl -SL "http://$PHP_VERSION.tar.bz2.asc"');
    const matcher = new Matcher(root);
  ...
  test("curlUseHttpsUrl valid", () => {
    const root = parseShell("curl -f https://host.com/");
    const matcher = new Matcher(root);
  ...
  test("wgetUseHttpsUrl valid", () => {
    const root = parseShell("wget https://host.com/");
    expect(new Matcher(root).match(wgetUseHttpsUrl)).toHaveLength(0);
  });
  ...
  test("configureShouldUseBuildFlag valid", () => {
    const root = parseDocker(`RUN ./configure \
    --build="$gnuArch" \
  ...
  test("gemUpdateSystemRmRootGem valid", () => {
    const root = parseDocker("RUN gem update --system;rm -rf /root/.gem;");
  ...
  test("yumInstallRmVarCacheYum valid", () => {
    const root = parseDocker("RUN yum install test;rm -rf /var/cache/yum");
  ...
  test("aptGetUpdatePrecedesInstall valid 2", () => {
    const root = parseDocker(
  ...
  test("aptGetUpdatePrecedesInstall 2", async () => {
    const root = praseFile("aptGetUpdatePrecedesInstall_fail");
    const matcher = new Matcher(root);
  ...
  test("gpgVerifyAscRmAsc", async () => {
    const root = parseDocker("RUN gpg --verify /usr/local/bin/gosu.asc");
    const matcher = new Matcher(root);
  ...
  test("aptGetInstallUseY valid", async () => {
    const root = parseDocker("RUN apt-get install -y test");
  ...
  test("aptGetInstallThenRemoveAptLists real scenario", async () => {
    const root = praseFile("complex-reprint");
    const matcher = new Matcher(root);
  ...
  test("aptGetInstallThenRemoveAptLists valid", () => {
    const root = parseDocker(
  ...
  test("apkAddUseNoCache valid with rm /var/cache/apk/*", () => {
    const root = parseDocker(
  ...
  test("sha256sumEchoOneSpaces valid", () => {
    const root = parseDocker(
  ...
  test("sha256sumEchoOneSpaces without echo valid", () => {
    const root = parseDocker(
